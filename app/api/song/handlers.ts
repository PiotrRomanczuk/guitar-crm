 ...
 */
export function validateMutationPermission(profile: { isAdmin?: boolean; isTeacher?: boolean } | null): boolean {
...
export async function createSongHandler(
  ...
  if (!validateMutationPermission(profile)) {
    return { error: 'Forbidden: Only teachers and admins can create songs', status: 403 };
  }
  ...
  try {
    ...
    if (err instanceof ZodError) {
      ...
      return { error: `Validation failed: ${JSON.stringify(fieldErrors)}`, status: 422 };
    }
...
export async function updateSongHandler(
  ...
  if (!validateMutationPermission(profile)) {
    return { error: 'Forbidden: Only teachers and admins can update songs', status: 403 };
  }
...
export async function deleteSongHandler(
  ...
  if (!validateMutationPermission(profile)) {
    return { error: 'Forbidden: Only teachers and admins can delete songs', status: 403 };
  }
