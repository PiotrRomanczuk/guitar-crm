 ...
 *
 * Tests OAuth connection, auth URL scope, token refresh,
 * calendar event fetching with date windows, and isGuitarLesson filtering.
 */
...
describe('Google Calendar OAuth (Journey 5)', () => {
  ...
  describe('getGoogleAuthUrl scope (Gap 7)', () => {
    it('requests full calendar scope (not calendar.readonly) plus drive.file', () => {
      ...
      // Verify readonly is NOT included
  ...
  describe('getCalendarEventsInRange (Gap 1)', () => {
    it('passes timeMin and timeMax to Google Calendar API', async () => {
      // Set up getGoogleClient's Supabase dependency
      const mockSupabase = {
        from: jest.fn().mockReturnThis(),
        select: jest.fn().mockReturnThis(),
        eq: jest.fn().mockReturnThis(),
        single: jest.fn().mockResolvedValue({
          data: { access_token: 'token', refresh_token: 'refresh', expires_at: 9999999999 },
          error: null,
        }),
      };
      (createClient as jest.Mock).mockResolvedValue(mockSupabase);
      ...
      const mockList = jest.fn().mockResolvedValue({
        data: { items: [{ id: 'e1', summary: 'Lesson' }] },
      });
  ...
  describe('isGuitarLesson filter', () => {
    it('filters events through isGuitarLesson based on Calendly description', () => {
      ...
      expect(filtered[0].description).toContain('Calendly');
      expect(filtered[1].description).toContain('Calendly');
    });
