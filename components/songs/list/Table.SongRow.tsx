      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <Link href={`/dashboard/songs/${song.id}`} className="font-medium text-foreground hover:text-primary truncate text-base">
              {song.title ?? 'Untitled'}
            </Link>
...
export function SongDesktopRow({
  ...
  deletingSongId, checkingId, onDeleteClick, onAssignClick, onRowClick,
}: SongDesktopRowProps) {
  const isBusy = deletingSongId === song.id || checkingId === song.id;
