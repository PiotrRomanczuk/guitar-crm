...
export function LessonAssignmentsList({ lessonId, assignments, canEdit }: LessonAssignmentsListProps) {
  return (
        ...
        <ul className="divide-y divide-gray-200 dark:divide-gray-700">
          {assignments.map((assignment) => (
            <li key={assignment.id} className="py-3 flex justify-between items-center">
              <div>
                <Link href={`/dashboard/assignments/${assignment.id}`} className="font-medium text-blue-600 hover:underline dark:text-blue-400">
                  {assignment.title}
                </Link>
                {assignment.due_date && (
