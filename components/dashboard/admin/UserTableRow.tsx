...
export function UserTableRow({ user, onEdit, onDelete }: UserTableRowProps) {
  ...
  return (
    <TableRow data-testid="user-table-row">
      <TableCell>
      ...
      <TableCell className="text-muted-foreground" data-testid="user-email">
        {user.email}
      </TableCell>
      <TableCell>
        <div className="flex flex-wrap gap-1" data-testid="user-roles">
          {roles.map((role) => (
      ...
      <TableCell>
        <Badge variant={user.isActive ? 'default' : 'destructive'} data-testid="user-status">
      ...
      <TableCell>
        <div className="flex items-center justify-end space-x-2">
          <Button variant="ghost" size="sm" onClick={onEdit} data-testid="user-edit-button">
            Edit
          </Button>
          <Button
