...
describe('Admin Profile Settings', () => {
	...
	context('Personal Information Management', () => {
		...
		it('should update personal information', () => {
			...
			// Update display name
			cy.get('body').then(($body) => {
				if ($body.find('[data-testid="display-name"], input[name="displayName"], input[name="display_name"]').length > 0) {
					cy.get('[data-testid="display-name"], input[name="displayName"], input[name="display_name"]')
						.clear()
			...
			cy.get('body').then(($body) => {
				if ($body.find('[data-testid="bio"], textarea[name="bio"]').length > 0) {
					cy.get('[data-testid="bio"], textarea[name="bio"]')
						.clear()
			...
			cy.get('body').then(($body) => {
				if ($body.find('[data-testid="phone"], input[name="phone"]').length > 0) {
					cy.get('[data-testid="phone"], input[name="phone"]')
						.clear()
			...
			// Save changes
			cy.get('[data-testid="save-profile"], button[type="submit"]')
				.click();
		...
		it('should validate email format', () => {
			...
			cy.get('body').then(($body) => {
				if ($body.find('[data-testid="email"], input[name="email"]').length > 0) {
					// Enter invalid email
					cy.get('[data-testid="email"], input[name="email"]')
						.clear()
						.type('invalid-email');
	...
	context('Profile Picture Management', () => {
		it('should display current profile picture or placeholder', () => {
			...
			cy.get('[data-testid="profile-picture"], .profile-avatar, img[alt*="profile"]')
				.should('be.visible');
		});
				...
				if ($body.find('[data-testid="upload-picture"], input[type="file"]').length > 0) {
					...
					cy.get('body').then(($body) => {
						if ($body.find('input[type="file"]').length > 0) {
							cy.get('[data-testid="upload-picture"], input[type="file"]')
								.selectFile('cypress/fixtures/profile-picture.jpg', { force: true });
						} else {
	...
	context('Password Management', () => {
		...
		it('should change password successfully', () => {
			...
			cy.get('[data-testid="current-password"], input[name="currentPassword"]')
				.type(passwordChange.currentPassword);

			// Enter new password
			cy.get('[data-testid="new-password"], input[name="newPassword"]')
				.type(passwordChange.newPassword);

			// Confirm new password
			cy.get('[data-testid="confirm-password"], input[name="confirmPassword"]')
				.type(passwordChange.confirmPassword);
	...
	context('Notification Preferences', () => {
		it('should display notification settings', () => {
			...
			cy.contains(/email.*notification|push.*notification|notification.*preference/i).should('be.visible');
		});
		...
		it('should update notification preferences', () => {
			...
			cy.get('body').then(($body) => {
				if ($body.find('[data-testid="email-notifications"], input[name="emailNotifications"]').length > 0) {
					cy.get('[data-testid="email-notifications"], input[name="emailNotifications"]')
						.check();

					// Toggle push notifications
					cy.get('[data-testid="push-notifications"], input[name="pushNotifications"]')
						.uncheck();
	...
	context('Error Handling', () => {
		...
		it('should handle network failures during password change', () => {
			cy.intercept('POST', '**/api/auth/change-password*', { forceNetworkError: true }).as('passwordError');
