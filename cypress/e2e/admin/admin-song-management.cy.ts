...
describe('Admin Song Management', () => {
    ...
    it('should display songs with all required information', () => {
      cy.get('body').then(($body) => {
        if ($body.find('table tbody tr, .song-item, .song-row').length > 0) {
          ...
          cy.get('table tbody tr, .song-item, .song-row')
            .first()
            .within(() => {
  ...
  context('Create New Song', () => {
    ...
    it('should create a new song with all fields', () => {
      ...
      cy.get('[data-testid="song-ultimate-guitar-link"], input[name="ultimate_guitar_link"]').type(
      ...
      cy.get('body').then(($body) => {
        if (
          $body.find('[data-testid="song-chords"], input[name="chords"], textarea[name="chords"]')
            .length > 0
        ) {
          cy.get('[data-testid="song-chords"], input[name="chords"], textarea[name="chords"]').type(
            testSong.chords
          );
        }
      });
    ...
    it('should handle duplicate song detection', () => {
      ...
      cy.get('[data-testid="song-key"], select[name="key"]').select('C');
      cy.get('[data-testid="song-ultimate-guitar-link"], input[name="ultimate_guitar_link"]').type(
        'https://www.ultimate-guitar.com/duplicate-song'
      );
      ...
      cy.get('[data-testid="song-key"], select[name="key"]').select('C');
      cy.get('[data-testid="song-ultimate-guitar-link"], input[name="ultimate_guitar_link"]').type(
        'https://www.ultimate-guitar.com/duplicate-song-2'
      );
  ...
  context('Edit Existing Song', () => {
    beforeEach(() => {
      ...
      cy.get('body').then(($body) => {
        if (!$body.text().includes(testSong.title)) {
          ...
          cy.get('[data-testid="song-key"], select[name="key"]').select(testSong.key);
          cy.get(
            '[data-testid="song-ultimate-guitar-link"], input[name="ultimate_guitar_link"]'
    ...
    it('should update song information', () => {
      ...
      // Update song fields
      cy.get('[data-testid="song-title"], input[name="title"]').clear().type(editedSong.title);

      cy.get('[data-testid="song-author"], input[name="author"]').clear().type(editedSong.author);

      cy.get('[data-testid="song-level"], select[name="level"]').select(editedSong.level);

      cy.get('[data-testid="song-key"], select[name="key"]').select(editedSong.key);
    ...
    it('should cancel edit without saving', () => {
      ...
      // Make changes
      cy.get('[data-testid="song-title"], input[name="title"]').clear().type('Should Not Save');
  ...
  context('Search and Filter', () => {
    it('should search songs by title', () => {
      ...
      cy.get(
        '[data-testid="song-search"], input[placeholder*="search"], input[type="search"]'
    ...
    it('should export songs data', () => {
      cy.get('body').then(($body) => {
        if ($body.find('[data-testid="export-songs"], button').length > 0) {
          ...
          cy.get('[data-testid="export-songs"], button')
            .contains(/export/i)
            .click();
  ...
  context('Error Handling', () => {
    ...
    it('should handle network failures during creation', () => {
      ...
      cy.get('[data-testid="song-key"], select[name="key"]').select('C');
      cy.get('[data-testid="song-ultimate-guitar-link"], input[name="ultimate_guitar_link"]').type(
        'https://www.ultimate-guitar.com/test'
      );
