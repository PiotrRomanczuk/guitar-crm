...
describe('Admin Assignments CRUD Workflow', () => {
  ...
  it('3. EDIT: should update the assignment', () => {
    ...
    // Click edit button
    cy.get('[data-testid="assignment-edit"], a[href*="/edit"], button:contains("Edit")', { timeout: 5000 })
      .first()
      .click({ force: true })
  ...
  it('5. DELETE: should remove the assignment', () => {
    ...
    cy.get('body').then($body => {
      if ($body.find('[role="dialog"], .modal').length > 0) {
        cy.get('[role="dialog"] button:contains("Delete"), .modal button:contains("Confirm")')
          .click({ force: true })
      }
    })
