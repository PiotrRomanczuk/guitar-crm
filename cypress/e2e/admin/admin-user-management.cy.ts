...
describe('Admin User Management', () => {
	...
	context('User List Management', () => {
		...
		it('should have search functionality', () => {
			...
			cy.get('[data-testid="user-search"], input[placeholder*="search"], input[type="search"]')
				.should('be.visible');
		});
	...
	context('Create New User', () => {
		...
		it('should create a new student user', () => {
			...
			// Fill out form
			cy.get('[data-testid="user-email"], input[name="email"]')
				.type(testUser.email);
			...
			// Set role to student
			cy.get('[data-testid="user-role"], select[name="role"], input[name="role"]')
				.then(($el) => {
					if ($el.is('select')) {
			...
			// Submit form
			cy.get('[data-testid="submit-user"], button[type="submit"]')
				.click();
		...
		it('should create a teacher user with correct permissions', () => {
			...
			// Fill form
			cy.get('[data-testid="user-email"], input[name="email"]')
				.type(teacherUser.email);
			cy.get('[data-testid="user-first-name"], input[name="firstName"]')
				.type(teacherUser.firstName);
			cy.get('[data-testid="user-last-name"], input[name="lastName"]')
				.type(teacherUser.lastName);
	...
	context('Edit Existing User', () => {
		...
		it('should open edit form for existing user', () => {
			...
			cy.contains('tr, .user-row', testUser.email)
				.within(() => {
		...
		it('should update user information', () => {
			...
			cy.url({ timeout: 10000 }).should('match', /\/users$|\/admin\/users$/);
			cy.contains(`${editedUser.firstName} ${editedUser.lastName}`, { timeout: 10000 })
				.should('be.visible');
		});
	...
	context('Delete User', () => {
		...
		it('should prevent deleting admin user', () => {
			...
			cy.contains('tr, .user-row', adminUser.email).then(($row) => {
				...
				cy.wrap($row).within(() => {
					cy.get('button').contains(/delete|remove/i).should(($btn) => {
	...
	context('Search and Filter', () => {
		...
		it('should filter users by role', () => {
			// Select role filter
			cy.get('[data-testid="role-filter"], select')
				.select('admin');
		...
		it('should clear search and filters', () => {
			...
			// Clear search
			cy.get('[data-testid="clear-search"], button, .clear-btn')
				.click();
