...
describe('Admin Lesson Management', () => {
		...
		it('should display lessons overview with proper layout', () => {
			...
			cy.get('body').then(($body) => {
				if ($body.find('table, .lessons-table, .lesson-list, .calendar-view').length > 0) {
					...
					cy.get('th, .table-header, .lesson-header').should('contain.text', /date|time|student|teacher/i);
				} else {
	...
	context('Create New Lesson', () => {
		...
		it('should create a new lesson with all details', () => {
			...
			// Select student
			cy.get('[data-testid="lesson-student"], select[name="student"], select[name="student_id"]')
				.select(1); // Select first available student

			// Select teacher  
			cy.get('[data-testid="lesson-teacher"], select[name="teacher"], select[name="teacher_id"]')
				.select(1); // Select first available teacher

			// Set date
			cy.get('[data-testid="lesson-date"], input[name="date"], input[type="date"]')
				.type(testLesson.date);

			// Set time
			cy.get('[data-testid="lesson-time"], input[name="time"], input[type="time"]')
				.type(testLesson.time);

			// Set duration
			cy.get('[data-testid="lesson-duration"], input[name="duration"], select[name="duration"]')
				.then(($element) => {
			...
			cy.get('[data-testid="lesson-notes"], textarea[name="notes"], input[name="notes"]')
				.type(testLesson.notes);

			// Set status (if available)
			cy.get('body').then(($body) => {
				if ($body.find('[data-testid="lesson-status"], select[name="status"]').length > 0) {
					cy.get('[data-testid="lesson-status"], select[name="status"]')
						.select(testLesson.status);
				}
			});

			// Submit form
			cy.get('[data-testid="submit-lesson"], button[type="submit"]')
				.click();
		...
		it('should validate required fields', () => {
			...
			cy.get('select:invalid, input:invalid, .error, .field-error, [aria-invalid="true"]')
	...
	context('Edit Existing Lesson', () => {
		...
		it('should open edit form for existing lesson', () => {
			...
			cy.get('table tbody tr, .lesson-item').first().within(() => {
		...
		it('should assign songs to a lesson', () => {
			...
			cy.get('body').then(($body) => {
				if ($body.find('[data-testid="assign-songs"], .song-assignment').length > 0) {
					...
					// Select a song
					cy.get('[data-testid="song-select"], select[name="song"]')
						.select(1); // Select first available song
		...
		it('should track song progress in lessons', () => {
			cy.get('body').then(($body) => {
				if ($body.find('.song-progress, [data-testid="song-progress"]').length > 0) {
					...
					cy.get('.song-progress select, [data-testid="song-progress"] select').first()
	...
	context('Lesson Filtering and Search', () => {
		it('should filter lessons by date range', () => {
			// Use date filters
			cy.get('[data-testid="start-date"], input[name="start_date"]')
				.type('2025-12-01');
			cy.get('[data-testid="end-date"], input[name="end_date"]')
				.type('2025-12-31');
		...
		it('should search lessons by notes content', () => {
			cy.get('[data-testid="lesson-search"], input[placeholder*="search"]')
				.type('chord');
		...
		it('should display lesson completion statistics', () => {
			cy.get('body').then(($body) => {
				if ($body.find('[data-testid="lesson-stats"], .analytics, .reports').length > 0) {
					...
					cy.get('[data-testid="completion-rate"], .completion-rate')
						.should('contain.text', /%|rate|completed/i);
				} else {
		...
		it('should show teacher performance metrics', () => {
			cy.get('body').then(($body) => {
				if ($body.find('.teacher-metrics, [data-testid="teacher-performance"]').length > 0) {
					cy.get('.teacher-metrics, [data-testid="teacher-performance"]')
						.should('be.visible');
				} else {
