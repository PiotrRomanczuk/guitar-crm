...
describe('Admin Songs CRUD Journeys', () => {
		...
		it('creates a new song via UI (if available)', () => {
			...
			cy.get('body').then(($body) => {
				if ($body.find('[data-testid="song-form"], form').length) {
					...
					cy.get('[data-testid="song-ug"], input[name="ultimate_guitar_link"]').clear().type(sampleSong.ultimate_guitar_link);
	...
	context('Read (List + Detail)', () => {
		it('lists songs and supports pagination/sort/filter', () => {
			cy.visit('/dashboard/admin/songs?page=1&sort=title&order=asc&level=intermediate', { failOnStatusCode: false });
			cy.get('body').then(($body) => {
	...
	context('Update', () => {
		it('updates a song via UI (if available)', () => {
			cy.visit('/dashboard/admin/songs', { failOnStatusCode: false });
			cy.get('body').then(($body) => {
				const editBtn = $body.find('[data-testid="song-edit"], a:contains("Edit"), button:contains("Edit")').first();
				if (editBtn.length) {
					cy.wrap(editBtn).click();
					cy.get('[data-testid="song-author"], input[name="author"]').clear().type('Updated Artist');
					cy.get('[data-testid="song-save"], button[type="submit"]').click();
		...
		it('deletes a song via UI (if available)', () => {
			...
			cy.get('body').then(($body) => {
				...
				if (delBtn.length) {
					...
					const confirm = $body.find('[data-testid="confirm-delete"], button:contains("Confirm")').first();
	...
	context('Validation & Edge Cases', () => {
		...
		it('rejects invalid inputs (API)', () => {
			cy.request({
				method: 'POST', url: '/api/admin/songs', failOnStatusCode: false,
	...
	context('Bulk Operations', () => {
		it('exports songs (if available)', () => {
			cy.request({ method: 'GET', url: '/api/admin/songs/export', failOnStatusCode: false }).then((res) => {
		...
		it('imports songs (if available)', () => {
			...
			cy.request({ method: 'POST', url: '/api/admin/songs/import', body: payload, failOnStatusCode: false }).then((res) => {
