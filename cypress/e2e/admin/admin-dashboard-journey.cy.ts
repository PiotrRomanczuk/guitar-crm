...
describe('Admin Dashboard Journey', () => {
	...
	context('Dashboard Overview', () => {
		...
		it('should show system statistics and metrics', () => {
			...
			cy.get('[data-testid^="stat-"], .stat-card, .metric-card').should('have.length.greaterThan', 0);
		...
		it('should display quick action buttons', () => {
			...
			cy.get('button, [role="button"], a').contains(/create|add|new|manage/i)
	...
	context('Dashboard Widgets and Cards', () => {
		...
		it('should display user statistics widget', () => {
			...
			cy.get('[data-testid^="user-"], .user-stat, .user-card')
		...
		it('should display lesson statistics widget', () => {
			...
			// Should show numbers or counts
			cy.get('[data-testid^="lesson-"], .lesson-stat, .lesson-card')
				.should('contain.text', /\d+/);
		});
		...
		it('should display song statistics widget', () => {
			...
			// Should show numbers or counts
			cy.get('[data-testid^="song-"], .song-stat, .song-card')
				.should('contain.text', /\d+/);
		});
