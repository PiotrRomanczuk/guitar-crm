...
describe('Admin Assignments CRUD Journeys', () => {
	...
	context('Create', () => {
			...
			cy.get('body').then(($b) => {
				if ($b.find('[data-testid="assignment-form"], form').length) {
					...
					if ($b.find('[data-testid="description"], textarea[name="description"]').length) {
						cy.get('[data-testid="description"], textarea[name="description"]').clear().type('Practice daily for 10 minutes.');
					}
		...
		it('creates a new assignment via API', () => {
			cy.request({ method: 'POST', url: '/api/admin/assignments', failOnStatusCode: false, body: {
				title: 'Scale practice', description: 'Major scales C-G-D', dueDate: tomorrow,
				studentId: '00000000-0000-0000-0000-000000000002', songId: '00000000-0000-0000-0000-000000000003'
			}}).then((res) => {
	...
	context('Update', () => {
		it('edits an assignment via UI (if available)', () => {
			cy.visit('/dashboard/admin/assignments', { failOnStatusCode: false });
			cy.get('body').then(($b) => {
				const edit = $b.find('[data-testid="assignment-edit"], a:contains("Edit"), button:contains("Edit")').first();
				if (edit.length) {
					cy.wrap(edit).click();
					if ($b.find('[data-testid="title"], input[name="title"]').length) {
						cy.get('[data-testid="title"], input[name="title"]').clear().type('Scale practice (updated)');
					}
		...
		it('edits an assignment via API', () => {
			cy.request({ method: 'GET', url: '/api/admin/assignments', failOnStatusCode: false }).then((list) => {
				const id = list.body?.data?.[0]?.id || '00000000-0000-0000-0000-000000000000';
				cy.request({ method: 'PUT', url: `/api/admin/assignments/${id}`, failOnStatusCode: false, body: { title: 'Updated' } })
					.then((res) => {
	...
	context('Delete', () => {
		...
		it('deletes an assignment via API (dummy id allowed)', () => {
			cy.request({ method: 'DELETE', url: '/api/admin/assignments/00000000-0000-0000-0000-000000000000', failOnStatusCode: false })
				.then((res) => {
	...
	context('Validation & Bulk', () => {
		...
		it('exports assignments (if available)', () => {
			cy.request({ method: 'GET', url: '/api/admin/assignments/export', failOnStatusCode: false }).then((res) => {
				expect(res.status).to.be.oneOf([200, 401, 403, 404, 500]);
			});
		});

		it('imports assignments (if available)', () => {
			const payload = { items: [{ title: 'Arpeggios', description: 'Basic triads', dueDate: tomorrow }] };
			cy.request({ method: 'POST', url: '/api/admin/assignments/import', body: payload, failOnStatusCode: false }).then((res) => {
