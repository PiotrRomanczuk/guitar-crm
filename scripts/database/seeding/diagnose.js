...
async function diagnose() {
  ...
  if (assignError) {
    ...
    assignments.forEach((a) =>
      console.log(
        `  - ${a.id.substring(0, 8)}... ${a.title} (lesson: ${
          a.lesson_id ? a.lesson_id.substring(0, 8) + '...' : 'null'
        })`
      )
    );
  }
  ...
  if (intError) {
    ...
    integrations.forEach((i) =>
      console.log(
        `  - ${i.id.substring(0, 8)}... ${i.provider} (user: ${i.user_id.substring(0, 8)}...)`
      )
    );
  }
  ...
  if (convError) {
    ...
    convs.forEach((c) =>
      console.log(
        `  - ${c.id.substring(0, 8)}... ${c.title} (${c.context_type}: ${
          c.context_id ? c.context_id.substring(0, 8) + '...' : 'null'
        })`
      )
    );
  }
  ...
  if (msgError) {
    ...
    msgs.forEach((m) =>
      console.log(
        `  - ${m.id.substring(0, 8)}... ${m.role} (conv: ${m.conversation_id.substring(0, 8)}...)`
      )
    );
  }
}
