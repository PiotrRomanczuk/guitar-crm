...
async function main() {
  ...
  await testStep('Step 4: Insert lesson_songs one by one', async () => {
    const lessonSongs = JSON.parse(fs.readFileSync(path.join(dataDir, 'lesson_songs.json'), 'utf8'));
    
    for (let i = 0; i < lessonSongs.length; i++) {
      const lessonSong = lessonSongs[i];
      console.log(`   [${i + 1}/${lessonSongs.length}] lesson_id: ${lessonSong.lesson_id.substring(0, 8)}...`);
  ...
  await testStep('Step 5: Insert lesson_history one by one', async () => {
    const lessonHistory = JSON.parse(fs.readFileSync(path.join(dataDir, 'lesson_history.json'), 'utf8'));
    
    for (let i = 0; i < lessonHistory.length; i++) {
      const history = lessonHistory[i];
      console.log(`   [${i + 1}/${lessonHistory.length}] ${history.change_type} - lesson_id: ${history.lesson_id.substring(0, 8)}...`);
  ...
  await testStep('Step 6: Insert assignment_history one by one', async () => {
    const assignmentHistory = JSON.parse(fs.readFileSync(path.join(dataDir, 'assignment_history.json'), 'utf8'));
    
    for (let i = 0; i < assignmentHistory.length; i++) {
      const history = assignmentHistory[i];
      console.log(`   [${i + 1}/${assignmentHistory.length}] ${history.change_type} - assignment_id: ${history.assignment_id.substring(0, 8)}...`);
