...
async function seedTestLessonSongs() {
  ...
  try {
    ...
    console.log(
      `✅ Successfully inserted ${insertedLessonSongs?.length || 0} lesson-song associations`
    );
    ...
    const { data: stats } = await supabase.from('lesson_songs').select('status').order('status');

    if (stats) {
      const statusCounts = SONG_STATUSES.reduce((acc, status) => {
        acc[status] = stats.filter((ls) => ls.status === status).length;
        return acc;
