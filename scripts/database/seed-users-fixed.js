...
const SERVICE_ROLE_KEY = process.env.NEXT_PUBLIC_SUPABASE_SERVICE_ROLE_KEY; // Changed to match .env.local
...
async function createDevUser(admin, user) {
	try {
		...
		if (!userId) {
			...
			if (listErr) {
				console.error(
					`Failed to list users for ${user.email}:`,
					listErr.message
				);
				return false;
			}
...
async function main() {
	...
	console.log('Service Role Key exists:', !!SERVICE_ROLE_KEY);
