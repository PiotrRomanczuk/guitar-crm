...
CREATE TABLE public.task_management (
  ...
  assignee_id uuid NULL REFERENCES profiles(user_id) ON DELETE
  SET
    NULL,
...
CREATE
OR REPLACE FUNCTION update_task_management_updated_at() RETURNS TRIGGER AS $ $ BEGIN NEW.updated_at = now();
RETURN NEW;
END;
$ $ language 'plpgsql';
CREATE TRIGGER update_task_management_updated_at BEFORE
UPDATE
  ON public.task_management FOR EACH ROW EXECUTE FUNCTION update_task_management_updated_at();
...
-- Sample data moved to seed.sql to avoid migration failures
  -- when no admin users exist yet