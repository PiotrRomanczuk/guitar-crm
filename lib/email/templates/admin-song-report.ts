  ...
  const renderMissingList = (
    ...
    const listItems = items
      ...
      <li style="margin-bottom: 4px;">
        <a href="${process.env.NEXT_PUBLIC_APP_URL}/dashboard/songs/${
          song.id
        }" style="color: #4f46e5; text-decoration: none; hover: underline;">
          ${song.title}
        </a> 
    ...
    const moreCount =
      items.length > 10
        ? `<li style="color: #6b7280; font-style: italic; margin-top: 4px;">...and ${
            items.length - 10
          } more</li>`
        : '';
      ...
      <div style="max-width: 600px; margin: 40px auto; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
        ...
        <div style="padding: 24px;">
          ...
          ${renderProgressBar(
            ...
            coverage.chords,
            counts.withChords,
            coverage.chords < 50 ? '#ef4444' : '#4f46e5'
          )}
          ${renderProgressBar(
            'YouTube Links',
            coverage.youtube,
            counts.withYoutube,
            coverage.youtube < 50 ? '#ef4444' : '#4f46e5'
          )}
          ${renderProgressBar(
            'Ultimate Guitar Links',
            coverage.ultimateGuitar,
            counts.withUltimateGuitar,
            coverage.ultimateGuitar < 50 ? '#ef4444' : '#4f46e5'
          )}
          ${renderProgressBar(
            'Gallery Images',
            coverage.galleryImages,
            counts.withGalleryImages,
            coverage.galleryImages < 50 ? '#ef4444' : '#4f46e5'
          )}
        </div>
        ...
        <div style="padding: 24px; background-color: #f9fafb; border-top: 1px solid #e5e7eb; text-align: center;">
          ...
          }/dashboard/songs" style="display: inline-block; background-color: #4f46e5; color: #ffffff; padding: 10px 20px; border-radius: 6px; text-decoration: none; font-weight: 500; font-size: 14px;">Go to Song Dashboard</a>
